import os
import sys
sys.path.append(os.path.abspath("../thirdparty/URetinex-Net"))
import argparse
import torch
...
class Inference(nn.Module):
    ...
    def forward(self, input_low_img):
        ...
        with torch.no_grad():
            ...
            save_path = os.path.join(self.opts.output, file_name.replace(name, "%s_%d_R"%(name, self.opts.ratio)))
            np_save_TensorImg(R, save_path)
            # save_path = os.path.join(self.opts.output, file_name.replace(name, "%s_%d_L"%(name, self.opts.ratio)))
            # np_save_TensorImg(L, save_path)
    ...
    def run(self, low_img_path):
        ...
        name = file_name.split('.')[0]
        print(name)
        img = Image.open(low_img_path).convert('L')
...
if __name__ == "__main__":
    ...
    parser.add_argument('--img_path', type=str, default="../img/image11.png")
    parser.add_argument('--output', type=str, default="../URetinexNet")
    # ratio are recommended to be 3-5, bigger ratio will lead to over-exposure 
    parser.add_argument('--ratio', type=int, default=5)
    # model path
    parser.add_argument('--Decom_model_low_path', type=str, default="../thirdparty/URetinex-Net/ckpt/init_low.pth")
    parser.add_argument('--unfolding_model_path', type=str, default="../thirdparty/URetinex-Net/ckpt/unfolding.pth")
    parser.add_argument('--adjust_model_path', type=str, default="../thirdparty/URetinex-Net/ckpt/L_adjust.pth")
